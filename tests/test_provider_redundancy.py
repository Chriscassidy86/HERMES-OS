...
def group(a,b=(101,),c=(101,)): return RedundantPublicProvider((Provider("Binance.US",a),Provider("Coinbase",b),Provider("Kraken",c)),conflict_tolerance_percent=5,latency_clock=lambda:0,clock=lambda:NOW)
class RedundancyTests(unittest.TestCase):
 ...
 def test_cooldown_and_recovery_probe(self):
  provider=Provider("Binance.US",(MarketDataError("x"),MarketDataError("x"),MarketDataError("x"),100)); fallback=Provider("Coinbase",(101,101,101,101,101,101)); third=Provider("Kraken",(101,101,101,101,101,101)); service=RedundantPublicProvider((provider,fallback,third),cooldown_cycles=2,latency_clock=lambda:0,clock=lambda:NOW)
 ...
 def test_stale_future_and_malformed_fail_closed(self):
  for timestamp,price in ((NOW.replace(year=2025),100),(NOW.replace(year=2027),100),(NOW,float("nan"))):
   provider=Provider("Binance.US",(100,)); provider.get_candle=lambda symbol,timeframe,t=timestamp,p=price:PublicCandle(provider.name,symbol,timeframe,t,p,1)
   service=RedundantPublicProvider((provider,),clock=lambda:NOW,latency_clock=lambda:0)
   with self.assertRaises(MarketDataError): service.get_candle("BTCUSDT")
