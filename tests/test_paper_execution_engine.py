...
class ExecutionTests(unittest.TestCase):
 def test_buy_sell_and_audit_context(self):
  ...
  sell=DecisionCycle((TrendSpecialist(),),clock=lambda:NOW).run(snap("Bearish",101)); closed=engine.execute(sell,101); self.assertEqual("PAPER_CLOSED",closed.status); self.assertEqual(1,len(portfolio.trades)); self.assertEqual(0,closed.trade_duration_seconds)
